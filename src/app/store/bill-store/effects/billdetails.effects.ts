...
export class BilldetailsEffects {
  ...
  Load_getbilldetails = this.actions$.pipe( ofType(actions.BilldetailsActionsTypes.Load_getbilldetails),
    switchMap(({ payload }) => {
      return this.http.post('http://localhost:8180/api/bill/billdetails/getbilldetails',payload).pipe(
        map((response: any) => new actions.Load_getbilldetailsSuccess({ billdetailss: response.data,cnt: response.cnt})),
